...
always @(*) begin
    ...
    else if ((ctr[reg_ctr:0]) == (depth*2-1)) begin
        next_state = OP_IDLE;
    end
end
    ...
    for (genvar k = 0; k < TP; k = k + 1) begin
        always @(posedge clk or posedge rst) begin
            if (rst) begin
                ...
                case (curr_state)
                    OP_SHUFFLE: begin
                        di00[k]       <= input_data[(TP-k)*LOGQ-1-:LOGQ];
                        dr00[k]       <= depth*(ctr<depth) + (((ctr & (depth-1)) & 1'd1)         << ((reg_ctr >= 1)  * ((0 >  magic_num) * (reg_ctr - (0 - magic_num))  + (0 <= magic_num) * (magic_num - 0)))) +
...
((((ctr & (depth-1)) & 12'd4095)>>11)<< ((reg_ctr >= 12) * ((11 > magic_num) * (reg_ctr - (11 - magic_num)) + (11 <= magic_num) * (magic_num - 11))));
                        dw00[k]       <= ctr;
                        de00[k]       <= 1;                        
...
generate
    ...
    for (genvar i = 0; i < TP; i = i + 1) begin
        always @(posedge clk) begin
            output_data[(TP-i)*LOGQ-1 -: LOGQ] <=  do00[(i & 1'd1) * (N1 >> 1) +
                                                        ((i & 2'd3) >> 1) * (N1 >> 2) +
                                                        ((i & 3'd7) >> 2) * (N1 >> 3) +
                                                        ((i & 4'd15) >> 3) * (N1 >> 4) +
                                                        ((i & 5'd31) >> 4) * (N1 >> 5) +

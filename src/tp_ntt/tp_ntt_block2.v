module tp_ntt_block2#(
    ...
    generate
        ...
        for (ntt_idx = 0; ntt_idx < (TP>>log_n2) ; ntt_idx = ntt_idx + 1) begin
            tp_ntt_core #(n2, LOGQ, BTF_LAT) NTT_units_pipelined(clk,rst, q_param_in ,NTT_param_in[(TP-n2*ntt_idx)*LOGQ-1-:n2*LOGQ], W_param_in[(bram_reg_size-(n2-1)*ntt_idx)*LOGQ-1-:(n2-1)*LOGQ], NTT_param_out[(TP-n2*ntt_idx)*LOGQ-1-:n2*LOGQ]);
        end
